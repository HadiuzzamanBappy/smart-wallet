    ...
    <div className="w-[80%] max-w-[280px] mx-auto flex flex-col justify-center items-center gap-6 relative z-50 animate-in fade-in zoom-in duration-1000">
      ...
      <div className="text-center relative z-10 flex flex-col items-center">
        ...
        <h2 className="text-[11px] font-bold text-amber-500/90 tracking-widest uppercase">
          {showManual ? (isLogin ? 'Access Vault' : 'Forge Key') : 'Authenticate'}
        </h2>
      </div>
      ...
      {showManual ? (
        <>
          {/* Manual Form */}
          <form onSubmit={handleSubmit} className="w-full space-y-2.5 relative z-10 flex flex-col items-center animate-in fade-in slide-in-from-bottom-2 duration-500">
            {!isLogin && (
          ...
          {/* Footer for Manual Form */}
          <div className="w-full flex flex-col items-center gap-3 relative z-10 pt-1 border-t border-stone-800/30 animate-in fade-in duration-500">
            <button
            ...
            <button
              ...
              className="text-[9px] text-stone-500 hover:text-emerald-500 font-bold uppercase tracking-widest transition-colors my-1"
            >
              ← Back to Google
            </button>
        ...
        <>
          {/* Main Google Auth Focus */}
          <div className="flex flex-col items-center justify-center gap-8 animate-in zoom-in-95 duration-500">
            <button
              onClick={handleGoogleLogin}
              disabled={loading}
              className="relative z-20 w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-gradient-to-br from-stone-800 via-stone-900 to-stone-950 border-[6px] border-stone-800/80 shadow-[0_8px_20px_rgba(0,0,0,0.6),inset_0_4px_10px_rgba(255,255,255,0.1)] flex items-center justify-center transition-all duration-300 group hover:scale-105 active:scale-95 hover:border-emerald-900/50"
            >
              <div className="absolute inset-2 rounded-full border-4 border-stone-950/40" />
              <div className="flex flex-col items-center justify-center text-stone-300 gap-2">
                <svg className="w-8 h-8 sm:w-10 sm:h-10 opacity-90 drop-shadow-sm group-hover:opacity-100 transition-opacity" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                </svg>
                <span className="text-[10px] sm:text-[11px] font-bold tracking-widest opacity-80 uppercase group-hover:text-emerald-500 transition-colors">Google</span>
              </div>
            </button>

            <div className="w-full flex flex-col items-center gap-4 relative z-10 pt-4 border-t border-stone-800/30">
              <button
