  ...
  return (
    <>
      <div className="flex flex-col h-full">
        <div className="px-4 py-4 space-y-3 border-b border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-900/60">
        ...
        <div className="divide-y divide-gray-100 dark:divide-white/5">
          {filteredTransactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center px-8">
              <div className="w-16 h-16 rounded-3xl bg-white/80 dark:bg-stone-900/40 border border-stone-200 dark:border-stone-800 flex items-center justify-center mb-6 opacity-40">
                <Filter className="w-8 h-8 text-stone-600 dark:text-stone-500 dark:text-stone-400" />
              </div>
              <p className="text-label text-stone-600 dark:text-stone-500 dark:text-stone-400">
            ...
            paginatedTransactions.map((transaction) => {
              ...
              return (
                <div key={transaction.id} className="px-4 py-3 hover:bg-white/80 dark:bg-stone-900/40 hover:bg-stone-100 dark:bg-stone-800 transition-all group border-b border-stone-200 dark:border-stone-800 last:border-0">
                  <div className="grid grid-cols-[auto_1fr_auto] gap-4 items-center">
                    {/* Left: Premium Avatar Compact */}
                    <div className="relative shrink-0">
                      <div className="w-10 h-10 rounded-2xl bg-white/80 dark:bg-stone-900/40 border border-stone-200 dark:border-stone-800 flex items-center justify-center text-lg shadow-sm">
                        <span className="opacity-80 drop-shadow-sm">{getCategoryEmoji(dc)}</span>
                      </div>
                      <div className={`absolute -right-1 -bottom-1 w-5 h-5 rounded-2xl flex items-center justify-center border-2 border-stone-200 dark:border-stone-800 shadow-sm ${isPositive ? 'bg-success-500 text-white' : 'bg-error-500 text-white'}`}>
                    ...
                    <div className="min-w-0">
                      <p className="text-label font-bold text-stone-800 dark:text-stone-200 truncate mb-1">{transaction.description}</p>
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-stone-50 dark:bg-stone-900/60 border border-stone-200 dark:border-stone-800">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      <div className={`text-label font-bold whitespace-nowrap ${isPositive ? 'text-success-600 dark:text-success-400' : 'text-stone-800 dark:text-stone-200'}`}>
                      ...
                      <div className="flex gap-1 transition-all">
                        {(transaction.type === 'repayment' || transaction.type === 'collection') && (
                          <button
                            ...
                            className="p-1 hover:bg-stone-100 dark:bg-stone-800 rounded-xl text-stone-600 dark:text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:text-stone-200 transition-all border border-transparent"
                            title="View details"
                          >
                        ...
                        <button
                          ...
                          className="p-1 hover:bg-error-500/10 rounded-xl text-stone-600 dark:text-stone-500 dark:text-stone-400 hover:text-error-500 transition-all border border-transparent"
                          title="Delete"
                        >
        ...
        {filteredTransactions.length > PAGE_SIZE && (
          <div className="py-6 px-5 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-stone-200 dark:border-stone-800 bg-white/80 dark:bg-stone-900/40">
            <div className="text-overline text-stone-600 dark:text-stone-500 dark:text-stone-400 uppercase tracking-widest">
              Audit Page <span className="text-stone-800 dark:text-stone-200 font-bold">{page}</span> of {totalPages}
            </div>
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 flex items-center justify-center rounded-xl text-overline transition-all duration-300 ${page === i + 1
                      ...
                      : 'text-stone-600 dark:text-stone-500 dark:text-stone-400 hover:bg-stone-100 dark:bg-stone-800'
                      }`}
                  >
                    {i + 1}
                  </button>
      ...
      >
        {adjustmentDetail ? (
          <div className="space-y-6">
            <GlassCard padding="p-5" className="flex items-center justify-between gap-4 !bg-white/80 dark:!bg-stone-900/40 border-stone-200 dark:border-stone-800">
              <div>
                <div className="text-overline text-stone-600 dark:text-stone-500 dark:text-stone-400 mb-1 uppercase tracking-widest">Net Adjustment</div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {adjustmentDetail.originalAmount !== undefined && (
                <div className="space-y-1.5 px-1">
                  <label className="text-overline text-stone-600 dark:text-stone-500 dark:text-stone-400 uppercase tracking-widest opacity-50">Principal</label>
                  <div className="text-label font-bold text-stone-800 dark:text-stone-200">{formatCurrency(adjustmentDetail.originalAmount, currency)}</div>
                </div>
              )}
              <div className="space-y-1.5 px-1">
                <label className="text-overline text-stone-600 dark:text-stone-500 dark:text-stone-400 uppercase tracking-widest opacity-50">Timestamp</label>
                <div className="text-label font-bold text-stone-800 dark:text-stone-200">{formatDate(adjustmentDetail.date || adjustmentDetail.createdAt)}</div>
              </div>
              <div className="space-y-1.5 px-1 col-span-2 pt-2 border-t border-stone-200 dark:border-stone-800">
                <label className="text-overline text-stone-600 dark:text-stone-500 dark:text-stone-400 uppercase tracking-widest opacity-50">Reference Entity</label>
                <div className="text-label font-bold text-stone-800 dark:text-stone-200 truncate">{adjustmentDetail.originalDescription || 'No Reference Found'}</div>
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-overline text-stone-600 dark:text-stone-500 dark:text-stone-400 uppercase tracking-widest opacity-50 px-1">Internal Audit Notes</label>
              <GlassCard padding="p-4" className="!bg-white/80 dark:!bg-stone-900/40 text-label text-stone-800 dark:text-stone-200 leading-relaxed border-none">
